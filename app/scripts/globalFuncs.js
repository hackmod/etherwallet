...
globalFuncs.printPaperWallets = function(strJson) {
    var walletWindow = window.open(
        "wallet",
        "_blank",
        "innerHeight = 360, innerWidth = 760, status = 1, resizable = 1"
    );
    let data = require("../layouts/printWallet.html").replace(
        "{{WALLETJSON}}",
        strJson
    );
...
globalFuncs.getSuccessText = function(str) {
    return (
        '<p class="text-center text-success"><strong> ' + str + "</strong></p>"
    );
};
globalFuncs.getDangerText = function(str) {
    return (
        '<p class="text-center text-danger"><strong> ' + str + "</strong></p>"
    );
};
...
globalFuncs.parityErrors = {
    ...
    "There are too many transactions in the queue\\. Your transaction was dropped due to limit\\. Try increasing the fee\\.":
        "PARITY_LimitReached",
    "Transaction fee is too low\\. It does not satisfy your node's minimal fee \\(minimal: (\\d+), got: (\\d+)\\)\\. Try increasing the fee\\.":
        "PARITY_InsufficientGasPrice",
    "Insufficient funds\\. The account you tried to send transaction from does not have enough funds\\. Required (\\d+) and got: (\\d+)\\.":
        "ERROR_17",
    "Transaction cost exceeds current gas limit\\. Limit: (\\d+), got: (\\d+)\\. Try decreasing supplied gas\\.":
...
globalFuncs.getEthNodeMsg = function(str) {
    ...
    else return this.getParityMsg(str);
};
...
globalFuncs.checkIfTokenExists = function(storedTokens, localToken) {
    return Boolean(
        storedTokens.find(
            token =>
                token.contractAddress === localToken.contractAdd &&
...
globalFuncs.saveTokenToLocal = function(localToken, callback) {
    try {
        if (!ethFuncs.validateEtherAddress(localToken.contractAdd))
            ...
            !globalFuncs.isNumeric(localToken.decimals) ||
            parseFloat(localToken.decimals) < 0
        )
            throw globalFuncs.errorMsgs[7];
        else if (
            !globalFuncs.isAlphaNumeric(localToken.symbol) ||
            localToken.symbol == ""
        )
...
globalFuncs.removeTokenFromLocal = function(symbol, tokenObj) {
    var storedTokens =
        globalFuncs.localStorage.getItem("localTokens", null) != null
            ? JSON.parse(globalFuncs.localStorage.getItem("localTokens", null))
            : [];
...
globalFuncs.getWalletPath = function(
    walletType = "trezor",
    nodeType = nodes.nodeTypes.CLO
) {
